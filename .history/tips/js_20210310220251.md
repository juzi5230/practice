...
## requestAnimationFrame
  ...
  + 在react中使用requestAnimationFrame和png序列实现动画效果

  ```js
    setImageUrl () { // 根据png序列的路径及数目拼接当前需要显示的png文件路径
        return 'images/panel/' + this.panelInfo.path + (this.time % this.panelInfo.pngNum + '').padStart(2, 0) + '.png'
    }
    renderImg () {
        if (typeof this.time === 'object') return // 实际使用时，出现了time初始化为null 的情况，做此处理
        this.inter++ // 间隔值
        if (this.inter % 5 === 0) { // 浏览器每渲染5次，更新一次图片， 即每秒12帧
            this.time++
        }
        if (!this.active && this.panelInfo.path) this.currentImagSrc = this.setImageUrl() // 根据实际情况添加条件限制当前图片地址的更新
    }
    animloop () {
        this.renderImg()
        requestAnimationFrame(this.animloop.bind(this)); // 使用requestAnimationFrame实现动画效果
    }
